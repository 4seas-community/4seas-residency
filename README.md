...
1. **Supabase**: create a new project → run `supabase/migrations/001_init.sql` in the SQL editor. Copy the project URL and service-role key.
...
5. **Vercel**: import the repo, add the same env vars. `vercel.json` registers the daily cron (`/api/cron/movein-guide`, 09:00 GMT+7); Vercel automatically sends `Authorization: Bearer $CRON_SECRET`.
...
- **Apply flow**: zod validation → honeypot (fake success) → rate limit (3/hour per hashed IP) → insert with status `submitted`. No confirmation email by design — the success page is the confirmation.
- **Admin**: `/admin/login` (shared long password + display name → HMAC-signed cookie) · `/admin` (list, filters, detail sheet, notes). Changing status to interview/accepted/rejected opens an email preview dialog (Update & send / Update without sending / Cancel). Status changes never roll back on email failure; every attempt is recorded in `email_log` with a Retry option.
