...
- **Decision/scheduling fields** (all admin-set, never emailed on change): `decided_after_interview` — chosen explicitly in the Accept/Reject variant submenu, defaults to "interview time set and past", null on non-terminal rows and legacy rows (rendered as the direct variant). `interview_scheduled_at` — the two INTERVIEW sub-stages (awaiting interview / awaiting decision, i.e. time unset-or-future vs past) are derived from it vs now (`lib/applications/utils.ts`), never stored, so they flow automatically as time passes. `confirmed_start_date` — admin-confirmed move-in date; `preferred_start_date` is never overwritten.
